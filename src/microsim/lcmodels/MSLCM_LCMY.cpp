...
// ===========================================================================
MSLCM_LCMY::MSLCM_LCMY(MSVehicle& v) :
    MSAbstractLaneChangeModel(v, LaneChangeModel::LCMY),
...
MSLCM_LCMY::~MSLCM_LCMY() {
    changed();
}


void
MSLCM_LCMY::initDerivedParameters() {
...
bool
MSLCM_LCMY::debugVehicle() const {
    return DEBUG_COND;
}


int
MSLCM_LCMY::wantsChange(
    int laneOffset,
...
double
MSLCM_LCMY::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
...
double
MSLCM_LCMY::_patchSpeed(double min, const double wanted, double max, const MSCFModel& cfModel) {
...
void*
MSLCM_LCMY::inform(void* info, MSVehicle* sender) {
...
double
MSLCM_LCMY::overtakeDistance(const MSVehicle* follower, const MSVehicle* leader, const double gap, double followerSpeed, double leaderSpeed) {
...
double
MSLCM_LCMY::informLeader(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
                           int blocked,
                           int dir,
                           const std::pair<MSVehicle*, double>& neighLead,
...
void
MSLCM_LCMY::informFollower(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
                             int blocked,
                             int dir,
                             const std::pair<MSVehicle*, double>& neighFollow,
                             double remainingSeconds,
...
double
MSLCM_LCMY::getExtraReservation(int bestLaneOffset) const {
    if (bestLaneOffset < -1) {
        return 20;
...
double
MSLCM_LCMY::anticipateFollowSpeed(const std::pair<MSVehicle*, double>& leaderDist, double dist, double vMax, bool acceleratingLeader) {
...
int
MSLCM_LCMY::slowDownForBlocked(MSVehicle** blocked, int state) {
...
void
MSLCM_LCMY::adaptSpeedToPedestrians(const MSLane* lane, double& v) {
...
double
MSLCM_LCMY::computeSpeedLat(double latDist, double& maneuverDist, bool urgent) const {
...
double
MSLCM_LCMY::getSafetyFactor() const {
    return 1 / myAssertive;
}

double
MSLCM_LCMY::getOppositeSafetyFactor() const {
    return myOppositeParam <= 0 ? std::numeric_limits<double>::max() : 1 / myOppositeParam;
}

bool
MSLCM_LCMY::saveBlockerLength(double length, double foeLeftSpace) {
...
std::string
MSLCM_LCMY::getParameter(const std::string& key) const {
...
void
MSLCM_LCMY::setParameter(const std::string& key, const std::string& value) {
    double doubleValue;
    try {
